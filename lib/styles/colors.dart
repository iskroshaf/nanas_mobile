// lib/styles/colors.dart

import 'package:flutter/material.dart';

const Color kBodyLightColor = Color(0xfff9fafb);
const Color kBodyDarkColor = Color(0xFF0D1B2A);
const Color kDialogDarkColor = Color(0xFF0D1B2A);
const Color kDialogLightColor = Color(0XFFFFFFFF);

const Color kTextColorHigh = Color(0xFF334155);
const Color kTextColorMedium = Color(0xFF64748b);
const Color kTextColorLow = Color(0xFFcbd5e1);

const Color kPrimaryColor = Color(0xFFf97316);
const Color kSecondaryColor = Color(0xff083344);

const Color kErrorColor = Color(0xFFB00020);
const Color kWarningColor = Color(0xFFFFA500);
const Color kInfoColor = Color(0xFF3B82F6);
const Color kSuccessColor = Color(0xFF00C853);

const Color kIconColor = Color(0xFF64748b);

const Color kWhiteColor = Color(0xFFFFFFFF);
const Color kTransparentColor = Color(0x00000000);

const Color kLineLightColor = Color(0xFFcbd5e1);
const Color kLineDarkColor = Color(0xFF64748b);
